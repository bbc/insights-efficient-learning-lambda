...
from test.fixtures.questions import NEXT_QUESTION, FORMATTED_QUESTION, VALID_QUESTION, \
...
def test_valid_event_no_questions_returns_random_question(mocker):
    random_question_mock = mocker.patch(
        'algorithm.choose_random_question')
...
def test_valid_event_with_results_returns_next_question(mocker):
    ...
    question_mock = mocker.patch('algorithm.choose_random_question')
