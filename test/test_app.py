...
def test_valid_event_with_results_returns_next_question(mocker):
    ...
    question_mock.assert_called()
    assert actual_question['statusCode'] == 200
    assert actual_question['body']
    assert actual_question['body']['nextQuestion'] == VALID_QUESTION
    assert actual_question['body']['results']
    assert len(actual_question['body']['results']) == 7
